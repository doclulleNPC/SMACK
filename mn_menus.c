...
{
  {
    ...
    {it_gap},
    {it_toggle,   "textured display",             "automap_textured"},
    {it_gap},
...
// core in-game actions the player can rebind; each points at the key_*
// keycode variable that g_game.c's input handler reads. key_use comes
// from g_game.h; the rest are externed here.
extern int key_up, key_down, key_left, key_right;
extern int key_strafeleft, key_straferight;
extern int key_fire, key_speed, key_strafe, key_autorun;
extern int key_lookup, key_lookdown, key_centerview, key_mlook;
extern int key_map;

static struct { const char *name; int *key; } keybinds[] =
{
  { "Fire",         &key_fire        },
  { "Use / Open",   &key_use         },
  { "Run",          &key_speed       },
...
static char keybind_desc[NUMKEYBINDS][40];
static char keybind_cmd [NUMKEYBINDS][20];

// human-readable name for a Doom keycode
static char *MN_KeyName(int c)
{
  static char buf[16];
  switch(c)
    {
      case 0:               return "---";
      case KEYD_RCTRL:      return "CTRL";
      case KEYD_RSHIFT:     return "SHIFT";
      case KEYD_RALT:       return "ALT";
      case KEYD_SPACEBAR:   return "SPACE";
      case KEYD_ENTER:      return "ENTER";
      case KEYD_TAB:        return "TAB";
      case KEYD_ESCAPE:     return "ESC";
      case KEYD_BACKSPACE:  return "BKSP";
      case KEYD_UPARROW:    return "UP";
      case KEYD_DOWNARROW:  return "DOWN";
      case KEYD_LEFTARROW:  return "LEFT";
      case KEYD_RIGHTARROW: return "RIGHT";
      case KEYD_INSERT:     return "INS";
      case KEYD_HOME:       return "HOME";
      case KEYD_END:        return "END";
      case KEYD_PAGEUP:     return "PGUP";
      case KEYD_PAGEDOWN:   return "PGDN";
      case KEYD_PAUSE:      return "PAUSE";
      case KEYD_CAPSLOCK:   return "CAPS";
      case KEYD_SCROLLLOCK: return "SCRLK";
      case KEYD_NUMLOCK:    return "NUMLK";
    }
  if(c >= KEYD_F1 && c <= KEYD_F10) { sprintf(buf, "F%d", c - KEYD_F1 + 1); return buf; }
  if(c == KEYD_F11) return "F11";
  if(c == KEYD_F12) return "F12";
  if(c > 32 && c < 127)
    { buf[0] = (c >= 'a' && c <= 'z') ? c - 32 : c; buf[1] = 0; return buf; }
  sprintf(buf, "#%d", c);
  return buf;
}

// --- the "press a key" capture widget (modelled on mn_misc.c popup_widget) ---

static int *bind_target = NULL;
static const char *bind_action = "";

static void MN_BindDrawer(void)
{
  char l2[64];
  ...
  MN_WriteText((unsigned char *)l2,
               (SCREENWIDTH - MN_StringWidth((unsigned char *)l2)) / 2, 96);
  MN_WriteText((unsigned char *)l3,
               (SCREENWIDTH - MN_StringWidth((unsigned char *)l3)) / 2, 112);
}
...
menuwidget_t binding_widget = { MN_BindDrawer, MN_BindResponder };

CONSOLE_COMMAND(mn_bindkey, 0)
{
  int i;
  if(!c_argc) return;
  i = atoi(c_argv[0]);
  if(i < 0 || i >= NUMKEYBINDS) return;
  bind_target = keybinds[i].key;
  bind_action = keybinds[i].name;
  current_menuwidget = &binding_widget;
}

menu_t menu_keybindings;    // populated at startup by MN_InitKeyBindings

void MN_KeyBindingsDrawer(void)
{
  int i;
  for(i = 0; i < NUMKEYBINDS; i++)
    sprintf(keybind_desc[i], "%-13s %s",
            keybinds[i].name, MN_KeyName(*keybinds[i].key));
}

static void MN_InitKeyBindings(void)
{
  int i;
  menu_keybindings.menuitems[0].type = it_title;
  menu_keybindings.menuitems[0].description = FC_GOLD "key bindings";
  menu_keybindings.menuitems[1].type = it_gap;
  for(i = 0; i < NUMKEYBINDS; i++)
    {
      sprintf(keybind_cmd[i], "mn_bindkey %d", i);
      menu_keybindings.menuitems[2+i].type = it_runcmd;
      menu_keybindings.menuitems[2+i].description = keybind_desc[i];
      menu_keybindings.menuitems[2+i].data = keybind_cmd[i];
    }
  menu_keybindings.menuitems[2+NUMKEYBINDS].type = it_end;
  menu_keybindings.x = 50;
  menu_keybindings.y = 12;
  menu_keybindings.selected = 2;      // first bindable row
  menu_keybindings.flags = mf_background | mf_leftaligned;
  menu_keybindings.drawer = MN_KeyBindingsDrawer;

  MN_KeyBindingsDrawer();             // fill descriptions before first draw
}
