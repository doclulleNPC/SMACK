...
{
  R_InitData();
  R_SetViewSize(screenSize+3);   // clamped to fullscreen at 11+ downstream
  R_InitPlanes();
  R_InitLightTables();
  R_InitSkyMap();
...
// 0..7 windowed view + status bar; 8 fullscreen; 9 fullscreen + GZDoom-style
// HUD; 10 the same HUD at 50%; 11 the vanilla status bar scaled to 50%
VARIABLE_INT(screenSize, NULL,                  0, 11, NULL);
...
CONSOLE_VARIABLE(screensize, screenSize, cf_buffered)
{
  ...
  if(gamestate == GS_LEVEL) // not in intercam
  {
     ...
     // screenSize 8..10 are all fullscreen (clamped to block 11 downstream);
     // the HUD variant is chosen from screenSize itself in HU_OverlayDraw
     R_SetViewSize (screenSize+3);
  }

  // fullscreen sizes (8+) show the overlay HUD; windowed sizes show the bar
  {
