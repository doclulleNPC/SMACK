...
int automap_grid = 0;
int automap_textured = 1;     // fill the map with floor textures (aidoom-style)
...
// Passed nothing, returns nothing
//
// Textured automap (aidoom-style): fill each explored subsector's floor area
// with its floor flat, sampled per pixel and shaded by the sector light level.
// Only explored (ML_MAPPED) subsectors are revealed, unless computer-map /
// IDDT is active -- matching how the walls reveal.
static void AM_drawFlats(void)
{
  extern int firstflat, numflats, *flattranslation, skyflatnum;
  extern seg_t *segs;
  extern lighttable_t **colormaps;

  int fx, fy;
  int lastpic = -2, lastlight = -1000;
  byte *flat = NULL;
  lighttable_t *cmap = colormaps[0];
  boolean reveal = ddt_cheating || (plr && plr->powers[pw_allmap]);

  // R_PointInSubsector (a BSP descent) is the expensive part; do it once per
  // BLKxBLK block, then fill the block sampling the flat per pixel.
  #define BLK 4
      ...
      for(fx = 0; fx < f_w; fx += BLK)
        {
          ...
          // reveal a subsector's floor only once one of its lines is mapped
          if(!reveal)
            {
              int j; boolean mapped = false;
          ...
          // R_PointInSubsector returns a leaf for any point, including the void
          // outside the map; a genuine interior point is on the front side of
          // every seg. If it's behind any wall, it's void -> leave background.
          for(i = 0; i < ss->numlines; i++)
            if(R_PointOnSegSide(wx, wy, &segs[ss->firstline + i]))
              break;
          if(i < ss->numlines) continue;

          sec = ss->sector;
          pic = sec->floorpic;
          if(pic == skyflatnum) continue;          // sky floor -> background

          if(pic != lastpic)
            {
              lastpic = pic;
              flat = (pic >= 0 && pic < numflats)
                   ? (byte *) W_CacheLumpNum(firstflat + flattranslation[pic], PU_CACHE)
                   : NULL;
            }
          if(!flat) continue;
...
void AM_Drawer (void)
{
  ...
  if (automap_textured)              // aidoom-style textured floor fill
    AM_drawFlats();
