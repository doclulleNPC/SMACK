...
// mbf21: P_CheckMeleeRange with the range supplied by the caller
// (Woof src/p_enemy.c:148), for A_MonsterMeleeAttack.
...
fixed_t viletryx;
fixed_t viletryy;
int     viletryradius;   // mbf21: A_HealChase radius (Woof src/p_enemy.c:1733)
...
// mbf21: A_VileChase's body, parameterized so A_HealChase can share it
// (Woof src/p_enemy.c:1713). Returns true if a corpse was raised.

static boolean P_HealCorpse(mobj_t* actor, int radius, statenum_t healstate,
                            int healsound)
{
  int xl, xh;
  int yl, yh;
...
void A_Nailbomb(mobj_t *thing)
{
  ...
  P_RadiusAttack(thing, thing->target, 128, 128);
...
// mbf21 codepointers
//
// Ported from Woof (src/p_enemy.c:2865-3271), which carries [XA]'s originals.
// Two deliberate differences: SMACK has no complevel system, so the `mbf21`
// guard on each pointer is dropped (a state only reaches these if a DEHACKED
// patch pointed it here), and damage has no MOD_* type, so A_MonsterMeleeAttack
// calls plain P_DamageMobj.
//
...
void A_SpawnObject(mobj_t *actor)
{
  ...
  an = actor->angle + (angle_t)(((long long)angle << 16) / 360);
...
void A_MonsterMeleeAttack(mobj_t *actor)
{
  ...
  if (range == 0)
    range = actor->info->meleerange ?
            actor->info->meleerange : MELEERANGE;
