...
// Misc. mobj flags
//
// mbf21 thing flags, from Woof (src/p_mobj.h:207). Set per thing type via a
// DEHACKED "MBF21 Bits" field, and per object at runtime by A_AddFlags /
// A_RemoveFlags.
...
struct mobj_s
{
  ...
  state_t*            state;
  unsigned long       flags;
  unsigned long       flags2;    // mbf21 flags, see mobjflag2_t
...
// itemrespawnque / itemrespawntime are file-static in p_mobj.c
// (only p_mobj.c references them). No extern declaration needed.
extern int iquehead;
extern int iquetail;
extern int gravity;
...
// mbf21
boolean P_SeekerMissile(mobj_t *actor, mobj_t **seekTarget, angle_t thresh,
                        angle_t turnMax, boolean seekcenter);
