...
# SDL3 backend. The original 1999 X11/svgalib targets are gone -- linux/i_xwin.c
# and linux/i_svga.c are still in the tree but no longer compiled. Single target:
# SDL3-only binary named `smmu`.
...
ifeq ($(SDL_CFLAGS),)
  ...
  SDL_LDFLAGS := -L/usr/local/lib -lSDL3 -pthread -lm
endif

# options common to all builds
# -fcommon: 1999-era C code relies on tentative (common) definitions of
# globals across multiple translation units. GCC 10+ defaults to -fno-common
# which breaks the SMMU build with multiple-definition errors.
CFLAGS_COMMON = -Wall -g -fcommon $(SDL_CFLAGS)

# new features; comment out what you don't want at the moment
CFLAGS_NEWFEATURES = -DDOGS

# debug options
# NOTE: -O0 (not -O2) -- the SMMU source has uninitialised reads that
# -O2 happily masks (e.g. d_main.c:1096 with wad_files[]), making
# the debug build "succeed" in misleading ways. -O0 keeps the bug
# visible and the trace honest.
#
# NOTE: -DRANGECHECK and -DINSTRUMENTED are intentionally OFF in
# the default DEBUG build. RANGECHECK tightens heap validation
# (Z_CheckHeap) by recomputing block sizes from HEADER_SIZE, but
# the 1999-era SMMU HEADER_SIZE=32 is too small for the
# INSTRUMENTED-augmented memblock_t (~48 bytes). Re-enable both
# only after auditing z_zone.c to use sizeof(memblock_t) instead
# of the hard-coded constant.
CFLAGS_DEBUG = -O0 -g
LDFLAGS_DEBUG =
...
CFLAGS  = $(CFLAGS_COMMON)  $(CFLAGS_$(MODE)) $(CFLAGS_NEWFEATURES)
LDFLAGS = $(LDFLAGS_COMMON) $(LDFLAGS_$(MODE)) $(SDL_LDFLAGS)
...
$(EXE): $(OBJS) $(O)/version.o
	$(CC) $(CFLAGS) $(LDFLAGS) $(OBJS) $(O)/version.o -o $@ $(SDL_LDFLAGS)
