...
// aidoom-style "50%" HUD: render the vanilla status bar full-size, snapshot it,
// restore the view behind it, then nearest-neighbour scale the bar to 50% and
// blit it centred along the very bottom. Reuses the normal bar draw so every
// widget stays correct. Used at the largest screen-size step (screenSize 11).
void ST_DrawScaled(void)
{
  ...
  int hh  = ST_HEIGHT << hires;         // bar height, native px
  int top = NH - hh;                    // bar's top row
  int bw  = ST_WIDTH << hires;          // bar width, native px
  int strip = NW * hh;
  int dw, dh, dx, dy, x, y;

  if(bw > NW) bw = NW;
  dw = bw/2; dh = hh/2;                 // scaled bar size (50%)
  dx = (NW - dw)/2; dy = NH - dh;       // centred along the bottom
  ...
  memcpy(vsave, screens[0] + top*NW, strip);        // 1) snapshot the view
  st_statusbaron = true;     // the widgets (numbers/face/arms) gate on this;
                             // ST_Drawer clears it in fullscreen, so force it on
  ST_doRefresh();                                   // 2) full bar -> screens[0]
  for(y=0; y<hh; y++)                               // 3) capture the bar
    memcpy(bcap + y*bw, screens[0] + (top+y)*NW, bw);
  memcpy(screens[0] + top*NW, vsave, strip);        // 4) put the view back
  for(y=0; y<dh; y++)                               // 5) scale the bar to 50%
  {
    byte *d = screens[0] + (dy+y)*NW + dx;
    byte *s = bcap + (y*2)*bw;
...
void ST_Drawer(boolean fullscreen, boolean refresh)
{
  ...
  if(fullscreen && !automapactive)
    {
      if(screenSize >= 11) ST_DrawScaled();   // 50% scaled bar (aidoom-style)
      return;
    }
